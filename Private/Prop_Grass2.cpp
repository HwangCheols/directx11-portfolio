...
CProp_Grass2* const CProp_Grass2::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CProp_Grass2* pInstnace = new CProp_Grass2(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CProp_Grass2::Clone(void* const& pArg) {
	CProp_Grass2* pInstnace = new CProp_Grass2(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CProp_Grass2::CProp_Grass2(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CProp_Grass2::CProp_Grass2(const CProp_Grass2& rhs)
	: CObject(rhs) {
}

void CProp_Grass2::Delete(void) {
	__super::Delete();
...
const HRESULT CProp_Grass2::Init_Create(void) {
	return S_OK;
}

const HRESULT CProp_Grass2::Init_Clone(void* const& pArg) {
	...
	hr |= __super::Add_Component(static_cast<_ubyte>(EScene::GAMEPLAY), TEXT("Model_grass2"), TEXT("Model"), reinterpret_cast<CComponent*&>(m_pModel));
...
const _ubyte CProp_Grass2::Update(const _double& dTimeDelta) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
	if (m_bSavePos == false) {
		m_bSavePos = true;
...
void CProp_Grass2::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
	m_pCollider->Update_Edit(1);
}

void CProp_Grass2::Late_Update(const _double& dTimeDelta) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
...
const HRESULT CProp_Grass2::Render(const CRenderer::EOrder& eOrder) {
	switch (eOrder) {
...
const HRESULT CProp_Grass2::Save_Data(Json::Value& root) {
	m_pTransform->Save_Data(root["Transform"]);
	return S_OK;
}

void CProp_Grass2::Load_Data(Json::Value& root) {
