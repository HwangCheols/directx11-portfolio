...
CProp_Grass* const CProp_Grass::Create(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork) {
	CProp_Grass* pInstnace = new CProp_Grass(pDevice, pContext, pNetwork);
	if (FAILED(pInstnace->Init_Create()))
		Safe_Release(pInstnace);
	return pInstnace;
}

CObject* const CProp_Grass::Clone(void* const& pArg) {
	CProp_Grass* pInstnace = new CProp_Grass(*this);
	if (FAILED(pInstnace->Init_Clone(pArg)))
		Safe_Release(pInstnace);
	return pInstnace;
}

CProp_Grass::CProp_Grass(ID3D11Device* const& pDevice, ID3D11DeviceContext* const& pContext, CNetwork* const& pNetwork)
	: CObject(pDevice, pContext, pNetwork) {
}

CProp_Grass::CProp_Grass(const CProp_Grass& rhs)
	: CObject(rhs) {
}

void CProp_Grass::Delete(void) {
	__super::Delete();
...
const HRESULT CProp_Grass::Init_Create(void) {
	return S_OK;
}

const HRESULT CProp_Grass::Init_Clone(void* const& pArg) {
	...
	hr |= __super::Add_Component(static_cast<_ubyte>(EScene::GAMEPLAY), TEXT("Model_grass"), TEXT("Model"), reinterpret_cast<CComponent*&>(m_pModel));
...
const _ubyte CProp_Grass::Update(const _double& dTimeDelta) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
	if (m_bSavePos == false) {
		m_bSavePos = true;
...
void CProp_Grass::Update_Edit(void) {
	m_pTransform->Update_Edit(0);
	m_pCollider->Update_Edit(1);
}

void CProp_Grass::Late_Update(const _double& dTimeDelta) {
	CGame_Instance* pGame_Instance = CGame_Instance::Get_Instance();
...
const HRESULT CProp_Grass::Render(const CRenderer::EOrder& eOrder) {
	switch (eOrder) {
...
const HRESULT CProp_Grass::Save_Data(Json::Value& root) {
	m_pTransform->Save_Data(root["Transform"]);
	return S_OK;
}

void CProp_Grass::Load_Data(Json::Value& root) {
